...
- **DP2 findings — RESOLVED:** TOCTOU MAJOR fixed (nudge() re-captures with no sleep, bails to `skipped_active` before any send-key) + Claude RV PASS (agent-peers 7486). Both MAJOR-to-VERIFY items live-confirmed (jolly-moose 7485: `pane_current_command`===`claude`/`codex`; `pane_title`===`peer:<name>` via `tab-title.ts`). MINOR bounded (`capture-pane -S -5`). 1 NIT observe-only (Codex literal→1s→Enter micro-gap; content-free, recheck-guarded).
- **PARKED HERE (bilateral, S310, commit `10e35c9`):** implementation COMPLETE + cross-RV'd behind `AGENT_PEERS_WAKE_MODE` (default off = zero runtime change). Full suite 118 pass / 0 fail, production tsc 0 errors.
- **REMAINING before any flag flip / log-only pilot (fresh-pair lane):** the **live active-model non-injection test** on disposable panes (acceptance gate #2 — prove no keystrokes injected during a genuinely active model turn; the unit TOCTOU test covers the race mechanically but not a real active turn). THEN a separate operator go-ahead for the log-only pilot → evidence → enable. Tracked: `MSAASA/tasks/s310-fix1-live-active-model-test.md`.
